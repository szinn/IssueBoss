use ib_api::grpc::{admin::artifact, admin_proto::ArtifactResponse};
...
async fn cmd_list(host: &str, port: u16, args: ListArgs) -> anyhow::Result<()> {
    ...
    print_artifacts(&resp.artifacts);
    Ok(())
}
...
fn print_artifact(a: &ArtifactResponse) {
    println!("Kind:    {}", a.kind);
    println!("Slug:    {}", a.slug.as_deref().unwrap_or("-"));
    println!("Body:    {}", a.body_json);
    println!("Created: {}", a.created_at);
    println!("Updated: {}", a.updated_at);
}

fn print_artifacts(artifacts: &[ArtifactResponse]) {
    println!("{:<16} {:<24} BODY", "KIND", "SLUG");
    println!("{}", "-".repeat(80));
    for a in artifacts {
        println!("{:<16} {:<24} {}", a.kind, a.slug.as_deref().unwrap_or("-"), a.body_json);
    }
}
